...
print(np.mean([face_detector(x) for x in human_files_short]))
print(np.mean([face_detector(x) for x in dog_files_short]))


# __回答:__
# 1. human_files的前100张图像中，能够检测到人脸的图像占比100%
# 2. dog_files的前100张图像中，能够检测到人脸的图像占比11%
...
# 
# __回答:__
# 1. 不太合理，受限于拍照时的设备、光线环境、角度等因素，不能保证照片中有清晰面部特征。
# 2. 可以考虑使用CNN卷积神经网络。
# 
...
print(sum(list(map(dog_detector, human_files_short))) / len(human_files_short))
print(sum(list(map(dog_detector, dog_files_short))) / len(dog_files_short))


# __回答:__
# 1. human_files的前100张图像中，能够检测到狗狗的图像占比0%
# 2. dog_files的前100张图像中，能够检测到狗狗的图像占比100%
...
# 
# __回答:__ 
# 1. 第一层为卷积层，filter为16，窗口宽高为2；第2层为最大池化层，池化窗口宽高为2；第3层为卷积层，filter为32，窗口宽高为2；设置dropout防止过拟合。卷积+池化之后，进行扁平化操作，然后添加两个全连接层，结点数目分别为256和133。卷积层的filter数目一般为2的幂，而且递增，这样设置是因为浅层的卷积层filter数目小，一般用于提取边缘、形状特征，深层的卷积层数目多，一般用于提取图片高级特征，而且这样设置训练好模型后可以方便以后模型迁移，特别是不相似的数据集，filter窗口大小选择2，是更细致提取图片的局部特征，；最大池化层放在卷积层后面，是对卷积层的输出进行降维，这样可以有效防止过拟合；之后添加扁平层是因为卷积+池化后已提取了图片的特征，不再需要图片的空间信息了；之后添加两层全连接层，是类似MLP进行分类和输出。采用ReLU函数是因为其导数大部分情况下为常数，在多层神经网络中，可以有效解决梯度因层级多而变的太小，导致反向传播无法修改权重参数的问题；最后一层采用softmax，是针对多分类，可以输出多个类别的概率。
...
model.fit_generator(datagen.flow(train_tensors, train_targets, batch_size), 
                    steps_per_epoch=train_tensors.shape[0] // batch_size, 
                    validation_data=(valid_tensors, valid_targets), 
                    validation_steps=valid_tensors // batch_size, 
...
# 
# __回答:__ 
# 观察了之前自定义CNN的训练输出，之所以准确率较低，是因为模型层次较少，欠拟合；使用迁移学习，采用预训练的ResNet-50模型提取特征。ResNet-50层次更多，并在ImageNet庞大的数据集进行了训练和优化，ImageNet下也有许多动物图片，而本次提供的数据集相对较小，特征与ImageNet有很多相似之处，使用ResnetNet-50提取的特征也可以用在本次提供的数据集中，所以只需要将ResetNet-50模型最后的全连接输出层删除掉，换成符合现在数据集要求的完全连接层输出，并使用预先训练的网络权重，可以大大减少训练时间和资源，也可以防止过拟合。添加一个全局平均池化层是因为数据集较小，减少特征在一定程度上可以防止过拟合。
# 
...
Resnet50_model.fit(train_Resnet50, train_targets, 
                   ...
                   epochs=10, batch_size=20, callbacks=[checkpointer])
...
### 然后返回此模型所预测的狗的品种
def predict_dog(filePath):
    dog_tensor = path_to_tensor(filePath)
    dog_feature = extract_Resnet50(dog_tensor)
    dog_prediction = np.argmax(Resnet50_model.predict( 
        dog_feature))
    return dog_names[dog_prediction].split('.')[1]
...
## 测试
filePath = '/data/dog_images/test/001.Affenpinscher/Affenpinscher_00003.jpg'
print(predict_dog(filePath))
...
def check_image(filePath):
    ...
    if face_detector(filePath):
        print('Hello human!')
        plt.imshow(img)
        print("You look like a...\n", predict_dog(filePath))
    elif dog_detector(filePath):
        print('Hello dog!')
        plt.imshow(img)
        print("You look like a...\n", predict_dog(filePath))
    else:
        print('Error! Don`t have human or dog')
        plt.imshow(img)
...
# __回答:__ 
# 1. 输出的结果还不错，狗的品种都识别正确，也能识别出人，但将一只猫识别为了人。
# 2. 模型的改进1：将人的图片和非人的图片组成数据集，通过CNN来识别人和非人图片
# 3. 模型的改进2：增加狗狗种类的训练集，提供更多的样本数据，尤其是带有噪音的图片，通过图像增强等方式，来提高模型的抗干扰能力，防止过拟合等问题
# 4. 可以更换激活函数，如Leakly ReLU, Randomized ReLU等，
# 5. 模型的改进3：可以尝试其它的CNN模型，如Inception等
...
## dog1是从网上下载的German shepherd dog
filePath = '../dog2.jpg'
check_image(filePath)
